...
| Conclusion | Note |
|------------|------|
| **No whole-repo rewrite** | Windows Demo path works; `frame` / `EnvelopeChannel` / `tab_strip` are already deep seams |
...
Reuse the `tab_strip` pattern (pure rules; Host + tests share the interface).

## Deepening candidates (recorded, deferred)

| Strength | Candidate |
|----------|-----------|
| **Strong (top)** | Keep `wid` behind the embed seam (`tabId` in the tab model) |
...
| Worth exploring | Land `IEmbedBackend` (Win-only first) — **bundle with wid move** |

Scan top recommendation: **wid → embed** first; peel Shell rules when touching tear-out.
...
| x11 / inproc / multi-client | wid locality + `IEmbedBackend` first |
