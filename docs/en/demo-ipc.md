...
| # | Decision |
|---|----------|
| 1 | Minimal commands + **title scheme A** (Host assigns a default tab title via `CreateSubWindow`; framework default `Tab{M}`, product may inject a factory) |
...
| UI | Host | IPC |
|----|------|-----|
| Home **Create Client** | `QProcess` → `Hello` → `MainWindowAdded` → `CreateSubWindow(title=Tab1)` | §3 |
| Client **新建窗口** | `Invoke("demo.request_new_window")` → `CreateSubWindow(TabM)` | Invoke + CreateSubWindow |
...
Host keeps `instance_index` / per-Client `content_index`, writes the default title (framework `Tab{M}`, or product `TabTitleFactory`) into `CreateSubWindow.title`; Client sets the window title and echoes it in `SubWindowAdded.title`.
