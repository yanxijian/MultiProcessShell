...
- [ ] Home（**demo_host** client area）→ Create Client → `Tab1` (framework default) embeds and fills the client area
- [ ] Client “新建窗口” → `Tab2` (same process, new tab)
...
- [ ] Home → Create Client again → new process / another `Tab1` (distinct accent OK)
