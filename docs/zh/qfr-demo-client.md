# Demo Client：QFluentRibbon ContentView

> **地位**：说明 Demo Client 的 ContentView 如何用无系统标题栏的 QFR `RibbonWindow` 作为嵌入 HWND。  
> **日期**：2026-07-26
...
| `mps_demo_client` | `qtheme::Engine` + `ThemeBridge` + frameless `RibbonWindow` ContentView |
...
- Ribbon：Home / Insert / View + QAT 钉选；中央「New Window」（UI 文案保留）
- 「New Window」仍走 `Invoke("demo.request_new_window")` → Host `createContentViewRequested` / `CreateSubWindow`
...
| 切换入口 | **demo_host** Home 客户区 Light/Dark；Client Ribbon Theme 组同等入口 |
...
| 同进程多 ContentView | 一进程一个 `Engine`，一次 `setColorScheme` 全窗生效 |
