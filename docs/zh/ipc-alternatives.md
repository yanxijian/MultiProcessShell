...
本文整理「六端跨平台 IPC 选型」类指南中的结论，并补充与 **MultiProcessShell（壳 + 嵌入）** 更相关的备选，供以后换栈或扩展时对照。  
...
| **Chromium Mojo** | 桌面强沙箱、句柄传递极强 | **不建议**中小团队作通用 IPC | 深度绑 GN/Chromium，剥离成本极高（与指南结论一致） |
...
1. **Cap'n Proto（全 C++）**  
   - 指南场景成立。  
   - 对本壳：仅当剖析证明 Protobuf 编解码成为热点、且 Client 长期纯 C++ 时再评估；迁移成本 = 新 IDL + 双栈过渡期。
...
5. **移动端单独成章**  
   六端指南里的 iOS/鸿蒙限制，对本仓库意味着：桌面 Demo **不能**假设可直接移植进程模型；备选 IPC 库「能编过六端」≠「壳形态在六端成立」。
...
外部参考（非仓库文档）：《跨平台（桌面与移动端 6 端）C++ IPC 选型与技术落地指南》——其中 Cap'n Proto / gRPC / Zenoh / 移动端沙箱与 Mojo 结论已吸收进本文 §3–§4。
